...
# Versions
ARG FLUENTD_SYSTEMD_VERSION="1.0.5"
ARG FLUENTD_LOKI_VERSION="1.2.16"
ARG FLUENTD_REWRITETAG_VERSION="2.4.0"
ARG FLUENTD_FIELDSPARSER_VERSION="0.1.2"
ARG FLUENTD_MULTIFORMATPARSER_VERSION="1.0.0"
ARG FLUENTD_GROKPARSER_VERSION="2.6.2"
ARG FLUENTD_RECORDMODIFIER_VERSION="2.1.1"
...
RUN apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends \
    ...
    gem install fluent-plugin-record-modifier -v "$FLUENTD_RECORDMODIFIER_VERSION" && \
